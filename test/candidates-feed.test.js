...
describe('Candidate Feed Page (feeds/candidates.html)', function () {
    const html = fs.readFileSync(path.join(ROOT, 'feeds', 'candidates.html'), 'utf-8');

    it('file exists', function () {
        assert.ok(fs.existsSync(path.join(ROOT, 'feeds', 'candidates.html')));
    });

    it('has correct title', function () {
        assert.ok(html.includes('<title>Candidate &amp; Official News — Comal County GOP Watch</title>'));
    });
    ...
    it('fetches candidate-news.json', function () {
        assert.ok(html.includes('/data/candidate-news.json'));
    });
    ...
    it('has page header with descriptive text', function () {
        assert.ok(html.includes('Candidate &amp; Official News'));
        assert.ok(html.includes('class="page-header"'));
    });
});

describe('Candidate Feed Page — Filtering Logic', function () {
    // Test the filtering approach by simulating with the data
    const data = JSON.parse(fs.readFileSync(path.join(ROOT, 'data', 'candidate-news.json'), 'utf-8'));

    it('candidate-news.json has entries', function () {
    ...
    it('"all" filter returns all entries', function () {
        // Simulating "all" filter — no filtering applied
        var filtered = data;
...
describe('Candidate Feed Page — Responsive Styles', function () {
    const html = fs.readFileSync(path.join(ROOT, 'feeds', 'candidates.html'), 'utf-8');
    ...
    it('does not have fixed widths that would cause horizontal scroll', function () {
        // Ensure max-width uses relative/responsive values
